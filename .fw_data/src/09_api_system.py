...
- `bash`: 1 command per call. No chaining (`;`, `&&`, `||`, pipe, redirect, subshell, `$`, multiline). Do not call executables by path; explicit paths must stay inside project.
  ...
  - File mutation: use `read`/`glob`/`grep` for inspection, `write`/`edit`/`delete`/`apply_patch` for mutation. Commands such as `rm`, `cp`, `mv`, `mkdir`, `touch`, `cat`, `head`, `tail`, `less`, `find`, `sed`, `curl`, `wget`, `apt`, `pkg`, and `sudo` are not allowed.
