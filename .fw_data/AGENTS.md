...
- Khi người dùng yêu cầu Canva, thiết kế slide/UI để import vào Canva hoặc PPTX thiên về visual → gọi `skill(name="canva")` trước. Skill này sẽ yêu cầu hỏi ý tưởng, chốt hướng thẩm mỹ (có thể tham khảo nguyên tắc trong `skill(name="design")` nếu cần chọn màu/type/layout từ đầu) rồi gọi tiếp `powerpoint` để tạo PPTX.
...
- Khi task là build hoặc sửa giao diện người dùng và đã có design system/component pattern sẵn trong project để match theo → gọi `skill(name="frontend-work")` trước khi bắt đầu code phần UI.
- Khi task là tạo landing page, dashboard, brand, hoặc bất kỳ UI nào cần một visual identity riêng và CHƯA có design system sẵn để bám vào (màu/font/layout phải tự chọn từ đầu) → gọi `skill(name="design")` trước. Không tự mặc định vào 1 trong các look phổ biến (nền cream+serif+accent đất nung, nền đen+accent neon, broadsheet layout) nếu brief không yêu cầu rõ.
